...
install_prezto() {
    ...
    zsh -c "$(cat << 'EOF'
        ...
        for rcfile in "${ZDOTDIR:-$HOME}"/.zprezto/runcoms/^(README.md|zshrc|zshenv|zpreztorc)(.N); do
...
install_herdr() {
    ...
    # home/.config/herdr/herdr-nav.sh (the script falls back to grep without jq).
